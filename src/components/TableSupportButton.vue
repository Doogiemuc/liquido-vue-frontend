<template>
	<div v-if="law">
	  <button v-if="readOnly || createdByCurrentUser" type="button" class="btn btn-default btn-xs disabled">
		...
		<button v-else type="button" class="btn btn-default btn-xs" @click="likeToDiscuss">
			<i class="far fa-thumbs-up"></i> {{law.numSupporters}}
		</button>
	</div>
</template>
...
<script>
/**
 * This is just the same as SupportButton.vue.  But DoogieTable passes the "law" as "row"
 */
export default {
	props: {
		row: { type: Object, required: false },   // <====== here is the differnrece to SupportButton.vue
