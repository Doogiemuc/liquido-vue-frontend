<template>
	<div v-if="!this.proposal" class="container" id="ProposalShow">
		<h1>Proposal</h1>
			...
			<div class="panel-body">
				...
				<div v-else-if="proposal.status === 'PROPOSAL'">
					...
					<ul v-if="!proposal.poll" class="startJoinList">
						<li><button type="button" @click="joinPoll" class="btn btn-sm btn-default" style="width:20ch">Join an existing poll</button> - You can add your proposal as an alternative suggestion to an existing poll.</li>
						<li><button type="button" @click="startNewPoll" class="btn btn-sm btn-default" style="width:20ch">Create a new poll</button> - Then others must join this new poll with their proposals, before a voting phase can start.</li>
					</ul>
				</div>
	...
	methods: {
		...
		startNewPoll() {
			this.$router.push({name: 'pollAdd', params: {
				proposal: this.proposal  // pass the proposal as full
			}})
		},
